...
public class Game : ISubmodule
{
    ...
    public string Description => "Play various games directly on Discord";

    public CommandData[] GetCommands(IServiceProvider provider)
    {
        List<ApplicationCommandOptionChoiceProperties> games = new();
        for (int i = 0; i < provider.GetRequiredService<GameManager>().Preloads.Length; i++)
        {
            var game = provider.GetRequiredService<GameManager>().Preloads[i];
#if !NSFW_BUILD
            if (!game.IsSafe())
            {
                continue;
    ...
    public async Task PlayAsync(IContext ctx)
    {
        ...
        if (gm.IsChannelBusy(ctx.Channel))
        ...
        else*/
        {
            ...
            var preload = gm.Preloads[(int)index];
            ...
            await gm.CreateGameAsync(ctx.Channel, game);
    ...
    public async Task LeaderboardAsync(IContext ctx)
    {
        ...
        float globalScore = 0;
        foreach (string tmpS in gm.AllGameNames)
        {
            var s = db.GetCacheName(tmpS);
    ...
    {
        var game = ctx.Provider.GetRequiredService<GameManager>().GetGame(ctx.Channel);

        if (game == null)
        {
            if (ctx.Provider.GetRequiredService<GameManager>().RemoveLobby(ctx.Channel.Id.ToString()))
            {
