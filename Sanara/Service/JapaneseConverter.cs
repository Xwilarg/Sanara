...
public class JapaneseConverter // TODO: Use external library instead?
{
    ...
    {
        _romajiToHiragana = JsonSerializer.Deserialize<Dictionary<string, string>>(File.ReadAllText($"{PathManager.Path}LanguageResource/Hiragana.json"))!;
        _hiraganaToRomaji = _romajiToHiragana.ToDictionary(x => x.Value, x => x.Key);
        _romajiToKatakana = JsonSerializer.Deserialize<Dictionary<string, string>>(File.ReadAllText($"{PathManager.Path}LanguageResource/Katakana.json"))!;
